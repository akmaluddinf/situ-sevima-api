...
const apiUrl = `${apiBaseUrl}live/krsmahasiswa`;
const namaApi = apiUrl.split('/').pop();
const namaTabel = "trn_krsmahasiswa"
        ...
        await Promise.all(dataFromApi.map(async (item) => {
          ...
          try {
            ...
            if (countResult[0].count === 0) {
              ...
              const insertQuery = `INSERT INTO ${namaTabel} (kode, idperiode, namakelas, nim, idmk, namamk, jenismatakuliah, sksmk, skspraktikum, skstatapmuka, skssimulasi, skspraktlap, nnumerik, nangka, nhuruf, krsdiajukan, krsdisetujui) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
              await connection.query(insertQuery, [
                kode, item.idperiode, item.namakelas, item.nim, item.idmk, item.namamk, item.jenismatakuliah, item.sksmk, item.skspraktikum, item.skstatapmuka, item.skssimulasi, item.skspraktlap, item.nnumerik, item.nangka, item.nhuruf, item.krsdiajukan, item.krsdisetujui
              ]);
