...
const apiUrl = `${apiBaseUrl}live/jalurpendaftaran`;
const namaApi = apiUrl.split('/').pop();
const namaTabel = "ref_jalurpendaftaran"
        ...
        await Promise.all(dataFromApi.map(async (item) => {
          ...
          try {
            ...
            if (countResult[0].count === 0) {
              ...
              const insertQuery = `INSERT INTO ${namaTabel} (kode, idjalurpendaftaran, namajalurpendaftaran) VALUES (?, ?, ?)`;
              await connection.query(insertQuery, [
                kode, item.idjalurpendaftaran, item.namajalurpendaftaran
              ]);
