...
const apiUrl = `${apiBaseUrl}live/pesertates`;
const namaApi = apiUrl.split('/').pop();
const namaTabel = "trn_pesertates"
        ...
        await Promise.all(dataFromApi.map(async (item) => {
          ...
          try {
            ...
            if (countResult[0].count === 0) {
              ...
              const insertQuery = `INSERT INTO ${namaTabel} (kode, kodependaftar, noujian, namapendaftar, nilaiseleksi, lulus, hadir, jenisprogram, namajenisseleksi, namaruang, tgljadwal, tgljadwalselesai, waktumulai, waktuselesai) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
              await connection.query(insertQuery, [
                kode, item.kodependaftar, item.noujian, item.namapendaftar, item.nilaiseleksi, item.lulus, item.hadir, item['jenis program'], item.namajenisseleksi, item.namaruang, item.tgljadwal, item.tgljadwalselesai, item.waktumulai, item.waktuselesai
              ]);
