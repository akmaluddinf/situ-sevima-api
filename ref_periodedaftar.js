...
const apiUrl = `${apiBaseUrl}live/periodedaftar`;
const namaApi = apiUrl.split('/').pop();
const namaTabel = "ref_periodedaftar"
        ...
        await Promise.all(dataFromApi.map(async (item) => {
          ...
          try {
            ...
            if (countResult[0].count === 0) {
              ...
              const insertQuery = `INSERT INTO ${namaTabel} (kode, idgelombang, idjalurpendaftaran, idperiodedaftar, idsistemkuliah, idperiode, namaperiodedaftar) VALUES (?, ?, ?, ?, ?, ?, ?)`;
              await connection.query(insertQuery, [
                kode, item.idgelombang, item.idjalurpendaftaran, item.idperiodedaftar, item.idsistemkuliah, item.idperiode, item.namaperiodedaftar
              ]);
