...
const apiUrl = `${apiBaseUrl}live/sistemkuliah`;
const namaApi = apiUrl.split('/').pop();
const namaTabel = "ref_sistemkuliah"
        ...
        await Promise.all(dataFromApi.map(async (item) => {
          ...
          try {
            ...
            if (countResult[0].count === 0) {
              ...
              const insertQuery = `INSERT INTO ${namaTabel} (kode, idsistemkuliah, namasistemkuliah) VALUES (?, ?, ?)`;
              await connection.query(insertQuery, [
                kode, item.idsistemkuliah, item.namasistemkuliah
              ]);
