...
const apiUrl = `${apiBaseUrl}live/rekappendaftar`;
const namaApi = apiUrl.split('/').pop();
const namaTabel = "trn_rekappendaftar"
        ...
        await Promise.all(dataFromApi.map(async (item) => {
          ...
          try {
            ...
            if (countResult[0].count === 0) {
              ...
              const insertQuery = `INSERT INTO ${namaTabel} (kode, idprodi, jenjang, namaprodi, tahunakademik, pendaftar, lulusseleksi) VALUES (?, ?, ?, ?, ?, ?, ?)`;
              await connection.query(insertQuery, [
                kode, item.idprodi, item.jenjang, item.namaprodi, item.tahunakademik, item.pendaftar, item.lulusseleksi
              ]);
