...
const apiUrl = `${apiBaseUrl}live/pesertaujian`;
const namaApi = apiUrl.split('/').pop();
const namaTabel = "trn_pesertaujian"
        ...
        await Promise.all(dataFromApi.map(async (item) => {
          ...
          try {
            ...
            if (countResult[0].count === 0) {
              ...
              const insertQuery = `INSERT INTO ${namaTabel} (kode, programstudi, kodemk, matakuliah, kurikulum, periode, kelas, sistemkuliah, kapasitas, pengawas1, pengawas2, jumlahpesertakelas, kelompok, ruang, jenispertemuan, tanggalujian, waktumulai, waktuselesai, nim, nama) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
              await connection.query(insertQuery, [
                kode, item.programstudi, item.kodemk, item.matakuliah, item.kurikulum, item.periode, item.kelas, item.sistemkuliah, item.kapasitas, item.pengawas1, item.pengawas2, item.jumlahpesertakelas, item.kelompok, item.ruang, item.jenispertemuan, item.tanggalujian, item.waktumulai, item.waktuselesai, item.nim, item.nama
              ]);
