...
const apiUrl = `${apiBaseUrl}live/gelombang`;
const namaApi = apiUrl.split('/').pop();
const namaTabel = "ref_gelombang"
        ...
        await Promise.all(dataFromApi.map(async (item) => {
          ...
          try {
            ...
            if (countResult[0].count === 0) {
              ...
              const insertQuery = `INSERT INTO ${namaTabel} (kode, idgelombang, namagelombang) VALUES (?, ?, ?)`;
              await connection.query(insertQuery, [
                kode, item.idgelombang, item.namagelombang
              ]);
