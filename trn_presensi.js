...
const apiUrl = `${apiBaseUrl}live/presensi`;
const namaApi = apiUrl.split('/').pop();
const namaTabel = "trn_presensi"
        ...
        await Promise.all(dataFromApi.map(async (item) => {
          ...
          try {
            ...
            if (countResult[0].count === 0) {
              ...
              const insertQuery = `INSERT INTO ${namaTabel} (kode, periodeakademik, programstudi, tahunkurikulum, kodemk, matakuliah, kelas, sistemkuliah, kelasmahasiswa, pertemuanke, tanggal, hari, waktumengajar, jenisjadwal, statusperkuliahan, presensi, nama, nim) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
              await connection.query(insertQuery, [
                kode, item.periodeakademik, item.programstudi, item.tahunkurikulum, item.kodemk, item.matakuliah, item.kelas, item.sistemkuliah, item.kelasmahasiswa, item.pertemuanke, item.tanggal, item.hari, item.waktumengajar, item.jenisjadwal, item.statusperkuliahan, item.presensi, item.nama, item.nim
              ]);
