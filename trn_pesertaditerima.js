...
const apiUrl = `${apiBaseUrl}live/pesertaditerima`;
const namaApi = apiUrl.split('/').pop();
const namaTabel = "trn_pesertaditerima"
        ...
        await Promise.all(dataFromApi.map(async (item) => {
          ...
          try {
            ...
            if (countResult[0].count === 0) {
              ...
              const insertQuery = "INSERT INTO trn_pesertaditerima (kode, kodependaftar, nama, noujian, prodipilihan, nilaiseleksi, seleksi, rek, cad, `set`, prodiditerima) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
              await connection.query(insertQuery, [
                kode, item.kodependaftar, item.nama, item.noujian, item.prodipilihan, item.nilaiseleksi, item.seleksi, item.rek, item.cad, item['set'], item.prodiditerima
              ]);
