...
const apiUrl = `${apiBaseUrl}live/datapendaftar`;
const namaApi = apiUrl.split('/').pop();
const namaTabel = "trn_datapendaftar"
        ...
        await Promise.all(dataFromApi.map(async (item) => {
          ...
          try {
            ...
            if (countResult[0].count === 0) {
              ...
              const insertQuery = `INSERT INTO ${namaTabel} (kode, kodependaftar, namapendaftar, alamat, asalsekolah, kota, pilihan1, pilihan2, prodiditerima, tmplahir, tgllahir, tgldaftar, namaayah, tlpayah, namaibu, tlpibu) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
              await connection.query(insertQuery, [
                kode, item.kodependaftar, item.namapendaftar, item.alamat, item.asalsekolah, item.kota, item.pilihan1, item.pilihan2, item.prodiditerima, item.tmplahir, item.tgllahir, item.tgldaftar, item.namaayah, item.tlpayah, item.namaibu, item.tlpibu
              ]);
