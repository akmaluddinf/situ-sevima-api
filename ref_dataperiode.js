...
const apiUrl = `${apiBaseUrl}live/dataperiode`;
const namaApi = apiUrl.split('/').pop();
const namaTabel = "ref_dataperiode"
        ...
        await Promise.all(dataFromApi.map(async (item) => {
          ...
          try {
            ...
            if (countResult[0].count === 0) {
              ...
              const insertQuery = `INSERT INTO ${namaTabel} (kode, kode_periode, nama_periode) VALUES (?, ?, ?)`;
              await connection.query(insertQuery, [
                kode, item.kode_periode, item.nama_periode
              ]);
