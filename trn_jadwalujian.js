...
const apiUrl = `${apiBaseUrl}live/jadwalujian`;
const namaApi = apiUrl.split('/').pop();
const namaTabel = "trn_jadwalujian"
        ...
        await Promise.all(dataFromApi.map(async (item) => {
          ...
          try {
            ...
            if (countResult[0].count === 0) {
              ...
              const insertQuery = `INSERT INTO ${namaTabel} (kode, programstudi, kodemk, matakuliah, kurikulum, periode, kelas, sistemkuliah, kapasitas, jumlahpesertakelas, kelompok, ruang, jenispertemuan, tanggalujian, waktumulai, waktuselesai, pengawas1, pengawas2) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
              await connection.query(insertQuery, [
                kode, item.programstudi, item.kodemk, item.matakuliah, item.kurikulum, item.periode, item.kelas, item.sistemkuliah, item.kapasitas, item.jumlahpesertakelas, item.kelompok, item.ruang, item.jenispertemuan, item.tanggalujian, item.waktumulai, item.waktuselesai, item.pengawas1, item.pengawas2
              ]);
