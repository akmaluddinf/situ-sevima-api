...
const apiUrl = `${apiBaseUrl}live/jadwalperkuliahan`;
const namaApi = apiUrl.split('/').pop();
const namaTabel = "trn_jadwalperkuliahan"
        ...
        await Promise.all(dataFromApi.map(async (item) => {
          ...
          try {
            ...
            if (countResult[0].count === 0) {
              ...
              const insertQuery = `INSERT INTO ${namaTabel} (kode, programstudi, kodemk, matakuliah, kurikulum, periode, kelas, sks, sistemkuliah, kapasitas, jumlahpesertakelas, pertemuan, tanggal, hari, waktumulai, waktuselesai, ruang, metodepembelajaran, jenispertemuan, nidn, nip, dosenpengajar, statuspengajaran, realisasimateri, rencanamateri, lokasi, kelasid, lastupdate, jadwalid, lastinsert) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
              await connection.query(insertQuery, [
                kode, item.programstudi, item.kodemk, item.matakuliah, item.kurikulum, item.periode, item.kelas, item.sks, item.sistemkuliah, item.kapasitas, item.jumlahpesertakelas, item.pertemuan, item.tanggal, item.hari, item.waktumulai, item.waktuselesai, item.ruang, item.metodepembelajaran, item.jenispertemuan, item.nidn, item.nip, item.dosenpengajar, item.statuspengajaran, item.realisasimateri, item.rencanamateri, item.lokasi, item.kelasid, item.lastupdate, item.jadwalid, item.lastinsert
              ]);
